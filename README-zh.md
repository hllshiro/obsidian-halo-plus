...
- 🔄 **自动同步**：配置文件夹，定时扫描文件变更（mtime + 内容 hash），内容有变化时自动发布
...
1. 从 [GitHub Releases](https://github.com/hllshiro/obsidian-halo-plus/releases) 下载最新版本的 `main.js`、`manifest.json`、`styles.css`
2. 将这三个文件复制到 `{vault}/.obsidian/plugins/halo-plus/`
...
1. 在设置中启用 "自动同步"
2. 添加需要同步的文件夹路径
3. 插件会定时扫描文件变更（默认每 30 秒），仅在内容有变化时自动发布
