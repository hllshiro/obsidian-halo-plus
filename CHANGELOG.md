...
### Added
- 全类型附件上传：支持图片、视频、音频、文档、压缩包等所有文件类型上传到 Halo
- 图片类型自定义：可配置哪些扩展名视为图片（默认 png/jpg/gif/webp 等）
- 附件大小限制：全局统一的上传大小限制（默认 100MB）
- 缓存自动迁移：旧版本的 `halo.images` 数据自动迁移到新的 `halo.assets` 格式
...
### Changed
- 附件处理设置重构：「图片处理」更名为「附件处理」，新增图片类型和大小限制配置
